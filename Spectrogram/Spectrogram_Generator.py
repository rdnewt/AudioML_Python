...
## Selected 2048 point fft to maintain resolution while "cutting out" the highest half of frequencies in the fft
## Spectrograms were pretty much empty above ~10kHz, so I decided to chop out that data below and maintain 512 points in frequency
pts_fft = 2048  # pts in each fft of the spectrogram, scipy only returns half of these, and then I half that again due to ^^^
...
dat_pts_5s = spectro_pts_5s*step+pts_overlap
...
for name in fnames:
    ...
    # fft will calculate for frequencies 0Hz-22050Hz, this clips that to 0Hz-11025Hz
    # again, very little data was present above ~10kHz, I think this will improve our resolution and remove empty data points
    f = f[0:pts_fft//4]
    Sxx = Sxx[0:pts_fft//4,:]
    ...
    spectro = np.array((f,t,Sxx))           # load spectrogram elements into a single numpy array
